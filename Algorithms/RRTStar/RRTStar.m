...
if (RRTStarTree_Iterations >= minIter)
    Qrand = robotTarget;
else
...
if (~CheckCollision(Qnear, Qnew, mapObstacles, map_size))
    ...
    for j = 1:K_nearest % Loop over entire tree checking for nearby points
        ...
        if (dist < RRTStarTree_Epsilon*2)
            % Line 9
            if (~CheckCollision(RRTStarTree(j).point, Qnew, mapObstacles, map_size))
                % Lines 10-12
    ...
    for j = 1:size(Qneighborhood)
        % Line 15
        if ((Qneighborhood(j) ~= Qnear_index) && ~CheckCollision(RRTStarTree(Qneighborhood(j)).point, Qnew, mapObstacles, map_size))
            if (RRTStarTree(Qneighborhood(j)).cost > (min_dist + RRTStarTree(Qnear_index).cost + pdist([RRTStarTree(Qneighborhood(j)).point;Qnew])))
                ...
                RRTStarTree(Qneighborhood(j)).link = Qnew_index;
    ...
    if (RRTStarTree_Threshold > pdist([Qnew; robotTarget]))
        isDone = true;
        RRTStarTree_Goals = [RRTStarTree_Goals; RRTStarTree(Qnew_index)];
    end
else 
    if (Qrand == robotTarget)
        ...
        minIter = RRTStarTree_Iterations + 50;
    end
end
